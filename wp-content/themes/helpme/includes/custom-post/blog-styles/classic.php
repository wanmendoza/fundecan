...
if ( is_sticky() ){
    ...
    $output .= '<article id="entry-' . get_the_ID() . '" class="blog-classic-entry sticky-post classic-'.$item_id.' helpme-isotop-item ' . $item_cat . '">';
}else{
	$output .= '<article id="entry-' . get_the_ID() . '" class="blog-classic-entry classic-'.$item_id.' helpme-isotop-item ' . $item_cat . '">';
}
    switch ($post_type) {
        ...
        case 'gallery':
			...
			if(class_exists('WPBakeryShortCode') && function_exists('custom_vc_init')){
            $output .= do_shortcode('[helpme_image_slideshow images="' . $attachment_ids . '" margin_bottom="0" image_width="' . $image_width . '" image_height="' . $image_height . '" effect="slide" animation_speed="700" slideshow_speed="7000" pause_on_hover="false" direction_nav="true"]');
            }
			$output .= '</div>';
        ...
        default:
            ...
            if (has_post_thumbnail()) {
                ...
                $output .= '<div class="hover-overlay"></div>';
                $output .= '<i class="helpme-theme-icon-plus hover-plus-icon"></i>';
                $output .= '</a></div>';
            }
            break;
