...
extract( shortcode_atts( array(
			...
			'expandable_icon' => 'helpme-theme-icon-plus',
...
$bg_stretch_class = ( $bg_stretch == 'true' ) ? 'helpme-background-stretch ' : '';
...
if($expandable != 'true'){
	if($layout_structure == 'full') {
		...
		if ($parallax_direction == 'both_axis_mouse' ) {
			$backgroud_image_alter = '<div class="helpme-mouse-parallax parallax-both-axis parallax-layer" style="'.$backgroud_image.'"></div>';
			$backgroud_image = '';

		} else if($parallax_direction == 'vertical_mouse'){
			$backgroud_image_alter = '<div class="helpme-mouse-parallax parallax-y-axis parallax-layer" style="'.$backgroud_image.'"></div>';
			$backgroud_image = '';

		} else if($parallax_direction == 'horizontal_mouse') {
			$backgroud_image_alter = '<div class="helpme-mouse-parallax parallax-x-axis parallax-layer" style="'.$backgroud_image.'"></div>';
			$backgroud_image = '';
...
echo  '<div'.$page_section_id.'  data-intro-effect="' . $intro_effect . '" class="page-section-'.$id.' ' . $page_intro_class . ' fullwidth-'.$full_width.' section-expandable-'.$expandable.' full-height-'.$full_height.' '.$bg_stretch_class.' helpme-video-holder helpme-page-section parallax-'.$parallax.' '.$visibility.' '.$el_class.'" data-direction="'.$parallax_direction.'">';

echo  wp_kses_post($backgroud_image_alter);


if ( $mask == 'true' && $layout_structure == 'full') {
	echo  '<div class="helpme-section-mask"></div>';
}
if ( !empty( $color_mask ) ) {
	$color_mask_css = ' style="background-color:'.$color_mask.';opacity:'.$mask_opacity.';"';
}
echo  '<div'.$color_mask_css.' class="helpme-section-color-mask"></div>';
...
if ( $bg_video == 'yes') {
	...
	if(!empty($poster_image)) {
			$video_output .= '<div style="background-image:url('.$poster_image.');" class="helpme-video-section-touch"></div>';	
	}

	$video_output .= '<div class="helpme-section-video"><video poster="'.$poster_image.'" muted="muted" preload="auto" loop="true" autoplay="true">';
...
echo  ($full_height == 'true' && $expandable == 'false') ? '<div class="helpme-page-section-loader sharp-slider-loading"><div class="helpme-preloader"><div class="helpme-loader"></div></div></div>' : '';


if($expandable == 'true') {
	echo  '<div class="expandable-section-trigger"><div class="helpme-expandable-wrapper"><div class="vc_col-sm-12  wpb_column column_container ">';
	
	echo  (!empty($expandable_txt)) ? '<div class="helpme-grid"><span class="align-'.$expandable_txt_align.'" style="color:'.$expandable_txt_color.';font-size:'.$expandable_txt_size.'px">'.$expandable_txt.'</span></div>' : '';
...
if($layout_structure == 'full') {
	if ( $full_width == 'true' ) {
		echo  '<div class="page-section-fullwidth vc_row-fluid page-section-content"><div class="helpme-padding-wrapper">'.wpb_js_remove_wpautop( $content ).'</div><div class="clearboth"></div></div>';
	} else {
		echo  '<div class="helpme-grid vc_row-fluid page-section-content"><div class="helpme-padding-wrapper">'.wpb_js_remove_wpautop( $content ).'</div><div class="clearboth"></div></div>';
	}
} else {
	echo  '<div class="helpme-half-layout '.$layout_structure.'_layout" style="background-image:url('.$bg_image.');">';
	echo  '<div>'.$video_output.'</div>';
	echo  '</div>';

	echo  '<div class="helpme-half-layout-container page-section-content '.$layout_structure.'_layout">'.wpb_js_remove_wpautop( $content ).'</div><div class="clearboth"></div>';
}
...
.page-section-'.$id. '.section-expandable-true:not(.active-toggle):hover .helpme-section-color-mask {
		opacity:'.($mask_opacity + 0.2).' !important;
}
';
if(!$expandable_txt == 'true'){
	$helpme_styles .= '.page-section-'.$id. ' .expandable-section-trigger i 
	{
...
echo  '<div class="helpme-main-wrapper-holder"><div class="theme-page-wrapper '.$layout.'-layout helpme-grid vc_row-fluid no-padding">';
