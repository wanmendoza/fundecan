...
if ($media_type == 'video') {
	if($video_host == 'self_hosted'){
		$videoContent .= '<div class="helpme-imagebox-video helpme-video-wrapper video-self-hosted"><div class="helpme-video-container"><video poster="" muted="muted" preload="auto" loop="true" >';
		...
		$videoContent .= '<div class="helpme-imagebox-item-image helpme-imagebox-video-preview"><img src="' . $preview_image . '" alt="" /></div>';

		$stream_source = 'self_hosted';
	}else{
		$videoContent .= '<div class="helpme-imagebox-video helpme-video-wrapper video-social-hosted"><div class="helpme-video-container">';
...
$output .= '<div id="image-box-' . $id . '" class="helpme-image-box autoplay-'.$video_autoplay.' rounded-'.$rounded_corner.' align-' . $align . ' ' . $visibility . ' ' . $animation_css . $el_class . '" style="max-width: ' . $media_width . 'px;' . $bg_color .$border_color. '" '.$stream_source_tag.'>';

$output .= '<div class="helpme-image-box-media" style="max-height:' . $media_height . 'px;">'; 
if ($media_type == 'video') {
	$output .= $videoContent;
} else {
	...
	if ($image_link == 'lightbox') {
		...
		$output .= '<div class="gallery-meta"><a class="helpme-lightbox" href="' . $image_src_array[0] . '" title="' . $image_title . '"><i class="helpme-theme-icon-plus"></i></a></div>';
	} else {
		$output .= '<div class="hover-overlay"></div>';
		$output .= '<div class="gallery-meta">';
		$output .= ($url != '') ? '<a class="" target="' . $target . '" href="' . $url . '" title="' . $image_title . '"><i class="helpme-theme-icon-next-big"></i></a>' : '';
		$output .= '</div>';
	}
