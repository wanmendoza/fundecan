...
global $woocommerce_loop, $woocommerce, $helpme_settings, $post, $wp_query, $helpme_dynamic_styles;
$helpme_styles = '';
...
if ( ! $product->is_in_stock() ) {
	...
	$out_of_stock_badge = '<span class="out-of-stock">'.esc_html__( 'OUT OF STOCK', 'helpme' ).'</span>';
        ...
        if($style == 'modern'){
            ...
            $output .= '<a href="'. apply_filters( 'out_of_stock_add_to_cart_url', get_permalink( $product->id ) ).'" alt="'. apply_filters( 'out_of_stock_add_to_cart_text', esc_html__( 'READ MORE', 'helpme' ) ).'" class="add_to_cart_button">'.esc_html__( 'PURCHASE', 'helpme' ).'</a>';
            $output .= $out_of_stock_badge;
            if ($product->is_on_sale()) :
            $output .= apply_filters('woocommerce_sale_flash', '<span class="onsale">'.esc_html__( 'SALE', 'helpme' ).'</span>', $post, $product);
            endif;
            $output .= '</div>';
        }
        ...
        if ( has_post_thumbnail() ) {
            ...
            $output .= '<img src="'.helpme_thumbnail_image_gen($image_output_src, $width, $height).'" class="product-loop-image" alt="'.get_the_title().'" title="'.get_the_title().'" itemprop="image">';
           ...
           /* $product_gallery = get_post_meta( $post->ID, '_product_image_gallery', true );
            ...
            $output .= '<img src="'.helpme_thumbnail_image_gen($image_hover_src, $width, $height).'" alt="'.get_the_title().'" class="product-hover-image" title="'.get_the_title().'" >';
            }*/
            ...
            $output .= '<span class="shop-tags-wrap">';
				...
				if ($product->is_featured()) :
					$output .= apply_filters('woocommerce_featured_flash', '<span class="onfeatured">'.esc_html__( 'FEATURED', 'helpme' ).'</span>', $post, $product);
				endif;
				$output .= $out_of_stock_badge;
			$output .= '</span>';
			}
		...
		$output .='<div class="product_bottom clearfix">';
            ...
            if ( ! $product->is_in_stock() ) {
                ...
                case "variable" :
                    ...
                    $icon_class = 'helpme-theme-icon-plus';
                    break;
                case "grouped" :
                ...
                case "external" :
                    ...
                    $icon_class = 'helpme-theme-icon-magnifier';
                    break;
                default :
...
if($pagination == "true"){
        ...
        $output .= helpme_woocommerce_pagination($query->max_num_pages);
        $output .= '</nav>';
}
$output .= '</div>';
    wp_reset_postdata();
