...
$posts = $categories = $pages =  $testimonials = $clients = $portfolio_posts = $sharp_posts = $tab_posts = $employees = $causes = $events = $pricing = $animated_columns = $infinite_animation =  $helpme_awesome_icons_list = $helpme_orderby = $helpme_product_orderby = $helpme_product_categories_orderby = $device_visibility = $css_animations = $product_cats = array();


global $helpme_settings;
if(class_exists('ReduxFrameworkPlugin')){
$skin_color = $helpme_settings['accent-color'];
}else{

$skin_color = '';
}
$target_arr = array( esc_html__( "Same window", "helpme" ) => "_self", esc_html__( "New window", "helpme" ) => "_blank" );


$add_device_visibility = array(
    "type" => "dropdown",
    "heading" => esc_html__("Visibility For devices", "helpme"),
    ...
    "description" => esc_html__("You can make this element invisible for a particular device (screen resolution) or set it to All to be visible for all devices.", "helpme")
);
...
$helpme_orderby = array(
    esc_html__( "Date", 'helpme' ) => "date",
    esc_html__( "Posts In (manually selected posts)", 'helpme' ) => "post__in",
    esc_html__( 'Menu Order', 'helpme' ) => 'menu_order',
    esc_html__( "post id", 'helpme' ) =>  "id",
    esc_html__( "title", 'helpme' ) =>  "title",
    esc_html__( "Comment Count", 'helpme' ) => "comment_count",
    esc_html__( "Random", 'helpme' ) => "rand",
    esc_html__( "Author", 'helpme' ) => "author",
    esc_html__( "No order", 'helpme' )  =>  "none",
);

$helpme_product_orderby = array(
    esc_html__( "Date", 'helpme' ) => "date",
    esc_html__( "Title", 'helpme' ) => "title",
    esc_html__( "Product ID", 'helpme' ) => "product_id",
    esc_html__( "Name", 'helpme' ) => "name",
    esc_html__( "Price", 'helpme' ) => "price",
    esc_html__( "Sales", 'helpme' ) => "sales",
    esc_html__( "Random", 'helpme' ) => "random",
);
$helpme_product_categories_orderby = array(
    esc_html__( "ID", 'helpme' ) => "id",
    esc_html__( "Count", 'helpme' ) => "count",
    esc_html__( "Name", 'helpme' ) => "name",
    esc_html__( "Slug", 'helpme' ) => "slug",
