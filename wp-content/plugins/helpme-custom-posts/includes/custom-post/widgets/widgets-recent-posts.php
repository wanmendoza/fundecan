...
class Designsvilla_Widget_Recent_Posts extends WP_Widget {

	function __construct() {
		$widget_ops = array( "classname" => "widget_posts_lists", "description" => "Displays the Recent posts" );
		WP_Widget::__construct( "recent_posts", HELPME_THEME_SLUG . " - Recent Posts", $widget_ops );
	...
	function widget( $args, $instance ) {
        ...
        <?php	$image_src_array = wp_get_attachment_image_src( get_post_thumbnail_id(), 'full', true );	
				$image_src = bfi_thumb( $image_src_array[ 0 ], array('width' => 90, 'height' => 70, 'crop'=>true));	
		 ?>
		
		 <img width="90" height="70" src="<?php echo helpme_thumbnail_image_gen($image_src, 90, 70); ?>" alt="<?php the_title(); ?>" />
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions['Designsvilla_Widget_Recent_Posts'] ) )
			delete_option( 'Designsvilla_Widget_Recent_Posts' );
